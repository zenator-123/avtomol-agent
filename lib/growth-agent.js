...
function buildCompetitiveStrategy(config, audits = [], commerce = {}) {
  const auditBySlug = new Map(audits.map((item) => [item.slug, item]));
  const orders = Number(commerce.orders30Days || 0);
  return (config.businessVerticals || []).map((vertical) => {
    const slugs = vertical.sites || [vertical.site].filter(Boolean);
    const issues = slugs.flatMap((slug) => auditBySlug.get(slug)?.seo?.issues || []);
    ...
    const keywords = config.competitiveIntelligence?.verticals?.[vertical.slug] || [];
    const action = vertical.slug === "vehicles"
      ? "Пази платените позиции, обновявай цена и наличност и показвай първо проверени автомобили с малък пробег."
      : vertical.slug === "fashion"
        ? "Изведи наличните модели с ясни размери, реални снимки, цена и доставка; свържи продуктовия каталог с безплатните канали."
        : "Създавай полезни местни страници по градове с реален процес, ориентировъчни фактори за цена и форма за запитване.";
    return {
      vertical: vertical.slug,
      name: vertical.name,
      opportunity: keywords[0] || vertical.name,
      evidence: critical + " критични и " + warning + " предупредителни SEO проблема; " + orders + " отчетени поръчки за 30 дни.",
      nextAction: action,
      expectedResult: "Повече релевантни посещения и измерими запитвания без платен разход.",
      risk: "Не публикувай дублиран, измислен или копиран текст; провери наличност и достъп преди външна промяна."
    };
  });
}

module.exports = { analyzeHtml, buildContentPlan, buildCompetitiveStrategy, summarizeLeads, trackedUrl };
