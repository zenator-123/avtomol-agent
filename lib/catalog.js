...
function extractModelTokens(normalizedMessage, makes, partTypes, tireSizes) {
  ...
  if (!makes.length && !partTypes.length && !tireSizes.length) {
    return [];
  }
...
function buildProductMeta(product) {
  ...
  return {
    haystack,
    partTypes: new Set(normalizeArray(product.partTypes).length ? product.partTypes : collectMatches(haystack, PART_TYPES)),
...
function scoreProduct(item, criteria, options = {}) {
  ...
  if (criteria.partTypes.length) {
    ...
    if (!partHits.length && options.strict !== false) {
      return null;
    }
    if (partHits.length) {
  ...
  if (criteria.makes.length) {
    const makeHits = criteria.makes.filter((make) => item.makes.has(make) || hasPhrase(item.haystack, make));
    if (!makeHits.length && options.strict !== false) {
      return null;
    }
    if (makeHits.length) {
...
function searchProducts(products, message, limit = 4) {
  ...
  if (strict.criteria.makes.length || strict.criteria.tireBrand || strict.criteria.tireSizes.length) {
    return [];
  }

  const relaxed = searchRanked(products, message, limit, { strict: false });
  return relaxed.results.map(toSuggestion);
}
