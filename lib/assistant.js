...
function buildAssistantInstructions(profile) {
  ...
  return [
    ...
    "Tyre source: tyre suggestions can come from the public Diana Tires catalog. For tyres, use size, season, brand, price, and availability from the current catalog context when present.",
...
function buildSmartNoMatchReply(message) {
  ...
  if (intent.isTire) {
    ...
    return [
      `За гуми${tireDetails ? ` ${tireDetails}` : ""} мога да ориентирам по AvtoMol и публичния каталог на Гуми Диана.`,
...
function buildMatchFooter(message, copy) {
  const intent = detectRequestIntent(message);
  if (intent.isTire) {
    return `За поръчка през AvtoMol или проверка по Гуми Диана изпратете във Viber/WhatsApp ${VIBER_PHONE}: размер, сезон, марка и брой гуми.`;
  }
...
function generateFallbackReply({ matches, message, profile }) {
  ...
  const picks = matches.slice(0, 3).map((product) => {
    const source = product.source === "diana-ltd.com" ? "Гуми Диана" : "AvtoMol";
