...
const { buildVehicleKnowledgeContext } = require("./vehicle-learning");
...
function buildAssistantInstructions(profile) {
  ...
  return [
    ...
    "You help customers choose available used vehicles and find auto parts, tyres, oils, filters, brakes, belts, suspension parts, batteries, and accessories.",
    `Roadside assistance is available 24/7 through AvtoMol.com. If the customer says they need roadside help, towing, fuel delivery, jump-starting, EV assistance, heavy-truck/bus recovery, or extraction from a ditch/ravine, stop the product-search flow and ask for exactly these three details: (1) current GPS location, (2) the problem, and (3) vehicle type/make/model. Tell them the completed request can be sent to Viber ${VIBER_PHONE}.`,
    "Never claim that the request was sent automatically. The website must show the customer a Viber button so they can open the chat and send the prepared request themselves.",
    "For vehicle sales, recognize and explain the actual vehicle data in the current catalog: make, model, year, mileage, fuel, transmission, price, equipment, condition, damage notes, incoming/stock number, and delivery time when present.",
    "Use learned vehicle knowledge only to compare and rank real current listings. Never copy an aggregated fact onto a specific vehicle and never invent missing vehicle data.",
    "Recommend only vehicles that are present and available in the current catalog. Offer up to 3 relevant choices and explain the trade-offs. If preferences are missing, ask for budget, vehicle type or make, fuel, transmission, and approximate year.",
    "For every vehicle offer, clearly state that delivery is included in the advertised price.",
    "Vehicle order process: after the customer confirms the order, AvtoMol.com issues a pro forma invoice. The customer must pay it; after payment, the vehicle is delivered within the timeframe stated in the offer or listing.",
    "Never invent a delivery date. If the current listing or offer has no timeframe, ask the customer to confirm the timeframe with AvtoMol.com.",
    ...
    "Never mention external supplier names to customers. Present all catalog matches and inquiry checks as AvtoMol service.",
    "Show real catalog items, article numbers, OEM numbers, prices, and tyre offers. Do not present SEO/category/model landing pages as product recommendations.",
    "If the customer asks by car make/model/year plus a part type and no exact real catalog item is found, do not invent compatibility. Ask for VIN/frame number or a Viber inquiry instead.",
    "For auto parts, prioritize exact SKU/article/OEM matches first, then part name matches. For tyres, prioritize size, season, brand, stock, and price.",
    "Bulgarian catalog vocabulary: акумулатори, масла, антифризи, течност за чистачки, чистачки, спирачна течност, спрейове и добавки, хигиена, предпазители, греси, изпускателна система, горивни маркучи, дифтунги, консумативи за гуми, силикони, скоби, каре кръстачка, части по двигателя, конектори, лагери, гуми, ролки, маншони, амортисьори за капак, клапани, зимни продукти, крушки, каре външно, закрепване на джанти, AdBlue. Treat these as product/category requests and answer in Bulgarian when the customer writes Bulgarian.",
...
function detectRoadsideIntent(message = "") {
  const normalized = normalizeText(message);
  return hasAny(normalized, [
    ...
    "dostavka na gorivo",
    "svrshi mi gorivoto",
    "elektromobil bez tok",
    "tir v kanavka",
    "kamion v kanavka",
    "avtobus v kanavka",
    "izvazhdane ot propast",
    "izteglyane ot propast",
    "izvazhdane ot dere",
    "izteglyane ot dere"
  ]);
}

function buildRoadsideRequestPrompt() {
  return [
    "Разбира се — ще подготвя заявка за пътна помощ 24/7.",
    "Моля, изпратете в едно съобщение:",
    "- точната GPS локация или адрес;",
    "- какъв е проблемът;",
    "- какъв е автомобилът — лек автомобил, бус, камион/ТИР, автобус или електромобил, плюс марка и модел ако ги знаете.",
    `След това ще оформя данните за изпращане по Viber на ${VIBER_PHONE}. При пострадали или непосредствена опасност първо се обадете на 112.`
  ].join("\n");
}
...
function buildPolicyContext(profile) {
  return [
    ...
    buildVehicleKnowledgeContext(profile.vehicleKnowledge),
  ]
    .filter(Boolean)
    .join("\n");
}
...
function detectRequestIntent(message) {
  ...
  const intent = {
    ...
    season: "",
    partLabel: "",
    isVehicleSale: hasAny(normalized, [
      "kupya kola",
      "kupya avtomobil",
      "tarsya kola",
      "tarsya avtomobil",
      "prodavate li koli",
      "prodavate li avtomobili",
      "avtomobili vтора upotreba",
      "avtomobili vtora upotreba",
      "nаличni avtomobili",
      "nalichni avtomobili",
      "kola za prodazhba",
      "avtomobil za prodazhba",
      "buy a car",
      "used car",
      "available vehicles",
      "gebrauchtwagen"
...
function buildSmartNoMatchReply(message) {
  ...
  if (intent.isVehicleSale) {
    return [
      "Мога да предложа налични автомобили от текущия каталог на AvtoMol.com.",
      "Напишете бюджет, предпочитана марка или тип автомобил, гориво, скоростна кутия и приблизителна година, за да сравня до 3 подходящи варианта.",
      "Доставката е включена в обявената цена. При поръчка се издава проформа фактура, която трябва да бъде платена; след плащането автомобилът се доставя в указания в офертата или обявата срок."
    ].join("\n");
  }
  ...
  if (intent.partLabel) {
    return [
      ...
      orderLine(),
      "Ако не знаете точната част, изпратете снимка или кратко описание на проблема. Ще проверим по каталози и налични партньорски доставки."
    ].join("\n");
  }
...
function buildMatchIntro(message, copy) {
  const intent = detectRequestIntent(message);
  if (intent.isVehicleSale) {
    return "Намерих подходящи налични автомобили:";
  }
  if (intent.isTire) {
...
function buildMatchFooter(message, copy) {
  const intent = detectRequestIntent(message);
  if (intent.isVehicleSale) {
    return "Доставката е включена в обявената цена. При поръчка AvtoMol.com издава проформа фактура; след плащането автомобилът се доставя в указания в офертата или обявата срок.";
  }
  if (intent.isTire) {
...
function generateFallbackReply({ matches, message, profile }) {
  ...
  if (detectRoadsideIntent(message)) {
    return buildRoadsideRequestPrompt();
  }
  ...
  const picks = matches.slice(0, 3).map((product) => {
    ...
    const identity = [product.sku || product.articleNumber || "", Array.isArray(product.oemNumbers) && product.oemNumbers.length ? `OEM: ${product.oemNumbers.slice(0, 3).join(", ")}` : ""].filter(Boolean).join(" | ");
    const details = [source, identity, product.summary || product.category || "", product.matchReason || ""].filter(Boolean).join(" - ");
